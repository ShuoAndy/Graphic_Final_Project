...
Material *SceneParser::parseMaterial(char type[]) {
    ...
    filename[0] = 0;
    bumpname[0] = 0;
    Vector3f diffuseColor(1, 1, 1), specularColor(0, 0, 0), attenuation(0, 0, 0);
...
moving_sphere *SceneParser::parseMovingSphere() {
    char token[MAX_PARSER_TOKEN_LENGTH];
    getToken(token);
    assert (!strcmp(token, "{"));
    getToken(token);
    ...
    assert (!strcmp(token, "}"));
    assert (current_material != nullptr);
    return new moving_sphere(center0, center1, time0, time1 , radius, current_material);
}
